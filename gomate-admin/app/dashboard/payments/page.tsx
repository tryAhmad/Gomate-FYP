...
export default function PaymentsPage() {
  ...
  const fetchStatistics = async () => {
    try {
      const response = await fetch(
        `${API_CONFIG.BASE_URL}/payments/statistics`
      );
      const result = await response.json();
  ...
  const markAsPaid = async (driverId: string) => {
    try {
      const response = await fetch(
        `${API_CONFIG.BASE_URL}/payments/${driverId}/mark-paid`,
        {
          method: "POST",
        }
      );
  ...
  const toggleAccountStatus = async (
    ...
    try {
      ...
      const response = await fetch(
        `${API_CONFIG.BASE_URL}/drivers/${driverId}/${endpoint}`,
        {
          method: "PATCH",
        }
      );
  ...
  const sendPaymentReminder = async (payment: DriverPayment) => {
    try {
      const response = await fetch(
        `${API_CONFIG.BASE_URL}/notifications/send-payment-reminder/${payment._id}`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            amount: payment.weeklyFee,
            dueDate: payment.lastPaymentDate,
          }),
        }
      );
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: `Payment reminder sent to ${getDriverName(payment)}.`,
        });
      } else {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Manage driver weekly fees and payment status
          </p>
        </div>
              ...
              <TableBody>
                {filteredPayments.map((payment) => (
                  <TableRow key={payment._id}>
                    ...
                    <TableCell>
                      <span
                        className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${getPaymentStatusColor(
                    ...
                    <TableCell className="font-medium">
                      PKR {payment.totalEarnings.toLocaleString()}
                    </TableCell>
                    <TableCell>
