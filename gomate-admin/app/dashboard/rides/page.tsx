...
import { Search, MoreHorizontal, Eye, MapPin, DollarSign, Clock } from "lucide-react"
...
export default function RidesPage() {
  ...
  const fetchRides = async () => {
    try {
      setLoading(true)
      const response = await fetch(`${API_CONFIG.BASE_URL}/statistics/all-rides`)
  ...
  const stats = {
    ...
    inProgress: rides.filter((r) => r.status === "started" || r.status === "accepted").length,
    totalRevenue: rides
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">Monitor and manage all platform rides</p>
        </div>
      </div>
                  ...
                  filteredRides.map((ride) => {
                    ...
                    return (
                      <TableRow key={ride._id}>
                        ...
                        <TableCell className="text-sm">{getPassengerName(ride)}</TableCell>
                        ...
                        <TableCell className="font-semibold">PKR {ride.fare.toFixed(2)}</TableCell>
