...
export default function UsersPage() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to fetch users (${response.status}). Is the backend running at ${API_CONFIG.BASE_URL}?`
        );
      }
      ...
      if (err instanceof TypeError && err.message.includes("fetch")) {
        setError(
          `Cannot connect to backend. Please ensure backend is running at ${API_CONFIG.BASE_URL}`
        );
      } else {
  ...
  const handleView = async (userId: string) => {
    ...
    try {
      const response = await fetch(
        `${API_CONFIG.BASE_URL}/passengers/${userId}`
      );
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const response = await fetch(
        `${API_CONFIG.BASE_URL}/passengers/${selectedUser._id}`,
        {
          method: "PATCH",
          headers: {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(
        `${API_CONFIG.BASE_URL}/passengers/${userToDelete._id}`,
        {
          method: "DELETE",
        }
      );
